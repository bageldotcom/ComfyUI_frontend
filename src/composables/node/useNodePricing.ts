...
/**
 * Async pricing function type for API calls
 */
type AsyncPricingFunction = (node: LGraphNode) => Promise<string>
...
const apiNodeCosts: Record<
  string,
  { displayPrice: string | PricingFunction | AsyncPricingFunction }
> = {
  FluxProCannyNode: {
  ...
  KlingImageGenerationNode: {
    displayPrice: (node: LGraphNode): string => {
      ...
      const nWidget = node.widgets?.find((w) => w.name === 'n') as IComboWidget
  ...
  KlingSingleImageVideoEffectNode: {
    displayPrice: (node: LGraphNode): string => {
      ...
      if (!effectSceneWidget) return '$0.28-0.49/Run (varies with effect scene)'
  ...
  OpenAIGPTImage1: {
    displayPrice: (node: LGraphNode): string => {
      ...
      if (!qualityWidget) return '$0.011-0.30 x n/Run (varies with quality & n)'
  ...
  BagelVideoNode: {
    displayPrice: async (node: LGraphNode): Promise<string> => {
      ...
      return await fetchBagelPricing({
        ...
        duration: Number(durationWidget.value),
        quality: String(qualityWidget?.value || '16:9'),
        generate_audio: Boolean(audioWidget?.value)
      })
    }
  },
  BagelImageNode: {
    displayPrice: async (node: LGraphNode): Promise<string> => {
      ...
      return await fetchBagelPricing({
        model: String(modelWidget.value),
        width: Number(widthWidget?.value || 1024),
        height: Number(heightWidget?.value || 1024),
        num_images: Number(numImagesWidget?.value || 1)
      })
    }
  },
  BagelSeeDanceNode: {
    displayPrice: async (node: LGraphNode): Promise<string> => {
      ...
      return await fetchBagelPricing({
        ...
        quality: String(qualityWidget?.value || '1080p')
      })
    }
  },
  BagelVeo3Node: {
...
export const useNodePricing = () => {
  /**
   * Get the price display for a node (handles both sync and async pricing)
   */
  const getNodeDisplayPrice = (node: LGraphNode): string | Promise<string> => {
    ...
    // If it's a function, call it and handle both sync and async
    if (typeof priceConfig.displayPrice === 'function') {
      try {
        const result = (
          priceConfig.displayPrice as (
            node: LGraphNode
          ) => string | Promise<string>
        )(node)

        // If async (returns Promise), handle errors
        if (result instanceof Promise) {
          return result.catch(() => '$0.00-5.00/Run')
        }

        // If sync, return directly
        return result
      } catch (error) {
        return ''
      }
  ...
  const getRelevantWidgetNames = (nodeType: string): string[] => {
    const widgetMap: Record<string, string[]> = {
      ...
      // Bagel nodes
      BagelVideoNode: ['model', 'duration', 'aspect_ratio', 'generate_audio'],
      BagelImageNode: ['model', 'width', 'height', 'num_inference_steps'],
      BagelSeeDanceNode: ['quality', 'duration'],
      BagelVeo3Node: ['model', 'duration', 'generate_audio'],
